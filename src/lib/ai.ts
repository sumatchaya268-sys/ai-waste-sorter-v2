...
export async function analyzeWaste(imageBase64: string, mode: 'general' | 'school') {
  ...
  const prompt = `
    ...
    **กฎสำคัญ (บังคับ):** 
    1. ภาพสิ่งของเครื่องใช้ทุกชนิด (เช่น เสื้อผ้าเก่า ของเล่น หนังสือ เครื่องใช้ไฟฟ้า เฟอร์นิเจอร์ ฯลฯ) ที่ผู้ใช้อาจต้องการโละทิ้ง ให้ถือว่าเป็น "สิ่งของที่ต้องการจัดการ" ห้ามตอบว่าไม่ใช่ขยะเด็ดขาด ให้จัดเข้าหมวด "ขยะทั่วไป" หรือ "รีไซเคิล" (ตามความเหมาะสม) และแนะนำวิธีจัดการใน disposalSteps เช่น "นำไปบริจาค", "ขายร้านรับซื้อของเก่า" เป็นต้น
    ...
    3. หากภาพเบลอมาก มืดเกินไป มองไม่รู้เรื่อง ห้ามเดามั่ว ให้จัดประเภท category เป็น "ไม่ใช่ขยะ" และใส่ warningMessage ว่า "ภาพเบลอหรือมืดเกินไป โปรดถ่ายใหม่อีกครั้งให้ชัดเจนขึ้นครับ"
    4. คุณต้องตอบกลับมาเป็น JSON เท่านั้น ห้ามพิมพ์ข้อความอื่นนอกกรอบ JSON เด็ดขาด
